...
class ThematiqueController extends Controller
{
    ...
    public function updateOrdreThematique(Request $Requests)
    {
        try {
    ...
    public function addCouche(Request $Requests)
    {
        try {
            ...
            foreach ($couches as $couche) {
                ...
                $newVariable = strtolower(preg_replace('/[^A-Za-z0-9]/', '', $oldVariable));
    ...
    public function getShemaTableCategorieFromIdCouche($id_couche, $sous_thematique)
    {
        if ($sous_thematique) {
            ...
            foreach ($themetique as $keythemetique) {
                ...
                foreach ($sousthemetique as $keysousthematique) {
                    ...
                    foreach ($couchethemetique as $keycouchethemetique) {
                        if ($keycouchethemetique->id == $id_couche) {
                            # code...
            ...
            foreach ($themetique as $keythemetique) {
                ...
                foreach ($couchethemetique as $keycouchethematique) {
                    if ($keycouchethematique->id == $id_couche) {
    ...
    public function createOSMTable($shema, $table, $sql, $type)
    {
        ...
        try {
            ...
            // $nombre = 0;
            // $table_exist = DB::select("SELECT EXISTS (SELECT FROM information_schema.tables WHERE  table_schema = '" . $shema . "' AND table_name   = '" . $table . "');");
            // if ($table_exist[0]->exists) {
            // 	$nombre_sql = DB::select('select count(*) as count from ' . $shema . '."' . $table . '"');
            // 	$nombre = $nombre_sql[0]->count;
            // }
            ...
            DB::select('CREATE INDEX ' . $table . '_geometry_idx ON ' . $shema . '."' . $table . '"' . '   USING GIST(geom)');
    ...
    public function change_nameCouche(Request $Requests)
    {
        try {
            ...
            $image_src = $Requests->input('nom_img_modife', null);
            $logo_src = $Requests->input('logo_src', null);
            $id = $Requests->input('key_couche', null);
            $sous_thematique = $Requests->input('sous_thematiques', null);
            $data['status'] = 'ok';
     ...
     */
    public function genrateSqlForLayer(int $id_cat, string $lim_adm, string $id_lim_adm, string $geomColum, $intersection,  $createTableOsm = true)
    {
